 ...
 *  @param kind is a bit mask of window 'widgets' present with the
...
short 
mt_x_wind_calc (short Type, short kind, short xkind, short InX,  short InY, short InW, short InH,
              short *OutX, short *OutY, short *OutW, short *OutH, short *global_aes)
{
	short *ptr;

	AES_PARAMS(0x7154,7,5,0,0);
                    
	ptr = aes_intin;
	*(ptr ++) = Type;						   /* aes_intin[0] */
	*(ptr ++) = kind;						   /* aes_intin[1] */
	*(ptr ++) = xkind;						   /* aes_intin[2] */
	*(ptr ++) = InX;						   /* aes_intin[3] */
	*(ptr ++) = InY;						   /* aes_intin[4] */
	*(ptr ++) = InW;						   /* aes_intin[5] */
	*(ptr)    = InH;						   /* aes_intin[6] */
