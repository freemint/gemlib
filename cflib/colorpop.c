...
static int fattrib[5], lattrib[6];	/* Sicherung fÅr FÅll- und Linienattribute */
...
static USERBLK popobj_userblk[MAX_COLORPOP] = { NULL, 0 };	/* Userblock-Liste fÅr alle Farbobjekte */
...
draw_non_3d (PARMBLK * parmblk)
{
	...
	{
		vsf_color (handle, WHITE);	/* erst ggf. vorher vorhandene Selektion weiû Åbermalen */
		grect_to_array ((GRECT *) & (parmblk->pb_x), pxy);
		v_bar (handle, pxy);
...
draw_3d (PARMBLK * parmblk)
{
	...
	if (color == T_NOCOLOR)
	{
		...
		if (state & SELECTED)	/* Das selektierte Kreuz ist weiter nach rechts unten verschoben, "eingedrÅckt" */
		{
			vsl_color (handle, BLACK);
