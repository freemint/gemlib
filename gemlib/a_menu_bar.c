 ...
 *								than 4.0 and mt_appl_getinfo() must
 *								indicate that this is feature is
 *								supported.
 *  @param global_aes global AES array
 *
 *  @return If \p me_mode is MENU_REMOVE (0) or MENU_INSTALL (1), the return
 *          value indicates an error condition where >0 means no
 *          error and 0 means an error occurred. In inquiry mode (\p me_mode
 *          = MENU_INQUIRE (-1)), mt_menu_bar() returns the application
 ...
 *  menu bar's owner within the scope of a mt_wind_update(BEG_UPDATE) 
 *  call to prevent the system from swapping
